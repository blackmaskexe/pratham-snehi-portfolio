...
export default function HeroProjectSectionPointer({
  startDelay = 4800,
  style,
}: Props) {
  const delaySec = (startDelay || 0) / 1000;

  // Always animate the bubble (reduced-motion handling removed by request)
  ...
  useEffect(() => {
    ...
    async function seq() {
      ...
      // subtle repeating lift: quick up, then pause so cycle ~= 1.5s
  ...
  return (
    <motion.div
      ...
      style={style}
      className="pointer-events-none flex flex-col items-center gap-2 z-20"
    >
      {/* Attention bubble — performs a subtle repeating jump to draw attention */}
      <motion.div
        // controlled by bubbleControls (entrance then repeating lift)
